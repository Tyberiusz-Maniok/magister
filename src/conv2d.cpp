...
Tensor& Conv2d::forward(Tensor& x) {
    ...
    Tensor& out = filters->batched_matmul(col, this->bias);
...
Tensor& Conv2d::backward(Tensor& grad, float lr) {
    ...
    Tensor& ainput_col = this->input_col->avg_grad();
    activation_fn.backward(agrad);

    agrad.reshape(1, 1, agrad.shape->c * agrad.shape->h, agrad.shape->w);
    Tensor& delta_w = agrad.matmul(ainput_col, nullptr, CblasNoTrans, CblasTrans);
    Tensor& col_grad = filters->matmul(agrad, nullptr, CblasTrans);
